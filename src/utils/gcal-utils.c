                                  ...
                                  const gchar     *action)
{
  ...
  if (desktop_environment)
    {
      if (g_strcmp0 (desktop_environment, "GNOME") != 0)
        {
...
/*
 * gtk_filter_list_model_set_watch_items() isn't available on the GTK4
 * version this build targets. This block replicates it by hand for
 * gcal_create_writable_calendars_model() below: watch each calendar's
 * "read-only" property directly via GtkExpressionWatch (core GTK4 API,
 * available since 4.0) and re-run the filter whenever it changes, keeping
 * the set of watches in sync as calendars are added to or removed from
 * the manager's calendars list.
 */
typedef struct
{
...
static void
on_writable_calendar_changed_cb (gpointer user_data)
{
  GtkFilter *bool_filter = GTK_FILTER (user_data);

  gtk_filter_changed (bool_filter, GTK_FILTER_CHANGE_DIFFERENT);
}

static void
reconcile_writable_calendars_watches (GcalWritableCalendarsWatch *watch_data)
{
  g_autoptr (GHashTable) still_present = NULL;
  GHashTableIter iter;
  gpointer calendar_key;
  gpointer watch_value;
  guint i, n_items;

  still_present = g_hash_table_new (NULL, NULL);
  n_items = g_list_model_get_n_items (watch_data->calendars);

  for (i = 0; i < n_items; i++)
    {
      g_autoptr (GcalCalendar) calendar = g_list_model_get_item (watch_data->calendars, i);

      g_hash_table_add (still_present, calendar);

      if (!g_hash_table_contains (watch_data->watches, calendar))
        {
  ...
  g_hash_table_iter_init (&iter, watch_data->watches);
  while (g_hash_table_iter_next (&iter, &calendar_key, &watch_value))
    {
      if (!g_hash_table_contains (still_present, calendar_key))
        {
          gtk_expression_watch_unwatch (watch_value);
...
static void
on_writable_calendars_watch_free (gpointer data)
{
  GcalWritableCalendarsWatch *watch_data = data;
  GHashTableIter iter;
  gpointer watch_value;

  g_signal_handler_disconnect (watch_data->calendars, watch_data->items_changed_id);

  g_hash_table_iter_init (&iter, watch_data->watches);
  while (g_hash_table_iter_next (&iter, NULL, &watch_value))
    gtk_expression_watch_unwatch (watch_value);

  g_hash_table_unref (watch_data->watches);
  gtk_expression_unref (watch_data->expression);
  g_free (watch_data);
}
...
gcal_create_writable_calendars_model (GcalManager *manager)
{
  ...
  bool_filter = gtk_bool_filter_new (gtk_expression_ref (expression));
  ...
  watch_data = g_new0 (GcalWritableCalendarsWatch, 1);
  watch_data->expression = expression;
  watch_data->bool_filter = GTK_FILTER (bool_filter);
