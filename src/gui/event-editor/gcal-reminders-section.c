...
update_new_alarm_icon (GcalRemindersSection *self)
{
  ...
  if (first_row == GTK_LIST_BOX_ROW (self->new_alarm_row))
    gtk_image_set_from_icon_name (GTK_IMAGE (self->new_alarm_row_icon), "add-reminder-symbolic");
  else
    gtk_image_set_from_icon_name (GTK_IMAGE (self->new_alarm_row_icon), "list-add-symbolic");
}
...
gcal_reminders_section_class_init (GcalRemindersSectionClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, GcalRemindersSection, new_alarm_row_icon);
